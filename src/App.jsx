  ...
  //     { id: 'electrical', name: 'Electrical', icon: '⚡', color: 'bg-yellow-400' },
  ...
  //       { id: 1, description: 'Cement 100 bags', amount: 45000, category: 'materials', date: '2024-01-15', supplier: 'RAM Cement Suppliers' },
  //       { id: 2, description: 'Steel rods 2 tons', amount: 180000, category: 'steel', date: '2024-01-14', supplier: 'Steel World' },
  //       { id: 3, description: 'Masonry work - Week 1', amount: 35000, category: 'labor', date: '2024-01-13', supplier: 'Local Labor Group' },
  //       { id: 4, description: 'Electrical wiring', amount: 28000, category: 'electrical', date: '2024-01-12', supplier: 'Electrical Works Co.' },
  //       { id: 5, description: 'Bricks 5000 pieces', amount: 75000, category: 'brick', date: '2024-01-11', supplier: 'Lakshmi Hardware' },
  //       { id: 6, description: 'Plumbing pipes & fittings', amount: 42000, category: 'plumbing', date: '2024-01-10', supplier: 'Plumbing Masters' },
  //     ];
  ...
  //   const budgetPercentage = (totalExpense / budget) * 100;

  //   const handleSubmit = (e) => {
  //     e.preventDefault();
  //     if (!description || !amount || !category || !supplier) return;
  ...
  //   const getCategory = (id) => categories.find(cat => cat.id === id) || categories[categories.length - 1];
  ...
  //           <div>
  //             <h1 className="text-2xl font-bold text-gray-800">Shareef's House</h1>
  //             <p className="text-gray-500 text-sm">Construction Expense Tracker</p>
  //           </div>
  ...
  //             <p className="text-2xl font-bold mt-1">₹{budget.toLocaleString()}</p>
  //           </div>
  ...
  //             <div 
  //               className={`h-full ${budgetPercentage > 80 ? 'bg-red-400' : budgetPercentage > 60 ? 'bg-yellow-400' : 'bg-green-400'} rounded-full`}
  ...
  //                     {((cat.total / totalExpense) * 100).toFixed(1)}% of total
  //                   </p>
  //                 </div>
  //               </div>
  //               <span className="font-bold text-gray-800">₹{cat.total.toLocaleString()}</span>
  //             </div>
  //           ))}
  //         </div>
  //       </div>
  ...
  //                   <span className="font-bold text-gray-800 block">₹{expense.amount.toLocaleString()}</span>
  ...
  //           <p className="text-gray-500 text-sm">{filteredExpenses.length} construction items</p>
  //         </div>
  ...
  //           className={`px-4 py-2 rounded-full whitespace-nowrap ${selectedCategory === 'all' ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-700'}`}
  //         >
  //           All
  //         </button>
  //         {categories.map(cat => (
  ...
  //             className={`px-4 py-2 rounded-full whitespace-nowrap flex items-center ${selectedCategory === cat.id ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-700'}`}
  //           >
  ...
  //                       <p className="font-semibold text-gray-800 mb-1">{expense.description}</p>
  ...
  //                   </div>
  //                   <div className="text-right">
  //                     <span className="text-lg font-bold text-gray-800 block">₹{expense.amount.toLocaleString()}</span>
  ...
  //     return (
  //       <div className="p-4">
  //         <h1 className="text-2xl font-bold text-gray-800 mb-6">Construction Reports</h1>
  ...
  //               <span className="font-bold text-gray-800">₹{budget.toLocaleString()}</span>
  //             </div>
  //             <div className="flex justify-between items-center">
  ...
  //               <span className="font-bold text-gray-800">{budgetPercentage.toFixed(1)}%</span>
  //             </div>
  //           </div>
  //         </div>
  ...
  //         {/* Category Breakdown */}
  //         <div className="bg-white rounded-2xl p-6 shadow-sm mb-6">
  //           <h2 className="font-semibold text-gray-800 mb-4">Detailed Category Breakdown</h2>
  //           <div className="space-y-4">
  //             {sortedCategories.map(cat => {
  //               const percentage = (cat.total / totalExpense) * 100;
  //               const budgetPerc = (cat.total / budget) * 100;
  //               return (
  //                 <div key={cat.id} className="space-y-2">
  //                   <div className="flex justify-between items-center">
  //                     <div className="flex items-center">
  //                       <div className={`w-8 h-8 rounded-full ${cat.color} flex items-center justify-center mr-3`}>
  ...
  //               ₹{(totalExpense / 30).toLocaleString()}
  //             </p>
  //           </div>
  //           <div className="bg-white rounded-xl p-4 shadow-sm">
  //             <p className="text-sm text-gray-500">Total Suppliers</p>
  //             <p className="text-xl font-bold text-gray-800 mt-1">
  //               {[...new Set(expenses.map(e => e.supplier))].length}
  //             </p>
  //           </div>
  //         </div>
  //       </div>
  ...
  //       .map(supplierName => {
  //         const supplierExpenses = expenses.filter(exp => exp.supplier === supplierName);
  //         const total = supplierExpenses.reduce((sum, exp) => sum + exp.amount, 0);
  //         return { name: supplierName, total, count: supplierExpenses.length };
  //       })
  //       .filter(s => s.total > 0)
  ...
  //           <p className="text-gray-500 text-sm">Track all construction suppliers</p>
  //         </div>

  //         {/* Suppliers List */}
  //         <div className="space-y-3">
  //           {supplierTotals.map((supplier, index) => (
  //             <div key={supplier.name} className="bg-white rounded-xl p-4 shadow-sm">
  //               <div className="flex items-center justify-between">
  //                 <div>
  ...
  //                       <p className="text-sm text-gray-500">{supplier.count} transactions</p>
  //                     </div>
  //                   </div>
  //                 </div>
  //                 <div className="text-right">
  //                   <span className="text-lg font-bold text-gray-800 block">₹{supplier.total.toLocaleString()}</span>
  //                   <span className="text-xs text-gray-500">
  //                     {((supplier.total / totalExpense) * 100).toFixed(1)}% of total
  //                   </span>
  //                 </div>
  //               </div>
  //             </div>
  ...
  //             <span className="font-semibold">Note:</span> All suppliers are tracked automatically when you add expenses
  //           </p>
  //         </div>
  //       </div>
  //     );
  ...
  //             className={`flex flex-col items-center p-2 ${activeTab === 'dashboard' ? 'text-orange-500' : 'text-gray-500'}`}
  //           >
  ...
  //           <button
  //             onClick={() => setActiveTab('expenses')}
  //             className={`flex flex-col items-center p-2 ${activeTab === 'expenses' ? 'text-orange-500' : 'text-gray-500'}`}
  //           >
  ...
  //             className={`flex flex-col items-center p-2 ${activeTab === 'reports' ? 'text-orange-500' : 'text-gray-500'}`}
  //           >
  ...
  //             className={`flex flex-col items-center p-2 ${activeTab === 'suppliers' ? 'text-orange-500' : 'text-gray-500'}`}
  //           >
...
function App() {
  ...
  useEffect(() => {
    ...
    if (!savedExpenses || JSON.parse(savedExpenses).length === 0) {
      const sampleExpenses = [
        { id: 1, description: 'Cement 100 bags', amount: 45000, category: 'materials', date: '2024-01-15' },
        { id: 2, description: 'Steel rods 2 tons', amount: 180000, category: 'steel', date: '2024-01-14' },
        { id: 3, description: 'Masonry work - Week 1', amount: 35000, category: 'labor', date: '2024-01-13' },
        { id: 4, description: 'Electrical wiring', amount: 28000, category: 'electrical', date: '2024-01-12' },
        { id: 5, description: 'Bricks 5000 pieces', amount: 75000, category: 'brick', date: '2024-01-11' },
        { id: 6, description: 'Plumbing pipes & fittings', amount: 42000, category: 'plumbing', date: '2024-01-10' },
      ];
      setExpenses(sampleExpenses);
      localStorage.setItem('constructionExpenses', JSON.stringify(sampleExpenses));
    }
  ...
  const handleSubmit = (e) => {
    ...
    let updatedExpenses;
    if (editingId) {
      updatedExpenses = expenses.map(exp => exp.id === editingId ? expenseData : exp);
    } else {
  ...
  const handleExportData = () => {
    ...
    link.href = url;
    link.download = `construction-expenses-${new Date().toISOString().split('T')[0]}.json`;
            ...
            <div key={cat.id} className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                <div>
                  <p className="font-medium text-gray-800">{cat.name}</p>
                  <p className="text-xs text-gray-500">
                    {totalExpense > 0 ? ((cat.total / totalExpense) * 100).toFixed(1) : 0}% of total
                  </p>
                </div>
              </div>
              <span className="font-bold text-gray-800">₹{cat.total.toLocaleString()}</span>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm mb-6">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-gray-600">Total Budget</span>
              <div className="flex items-center">
                <span className="font-bold text-gray-800 mr-2">₹{budget.toLocaleString()}</span>
                <button
                  onClick={handleBudgetEdit}
                  className="p-1 text-gray-400 hover:text-orange-500"
                >
